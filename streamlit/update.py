...
def update():
    ...
    # st.write(result)
    df = pd.DataFrame(result, columns=["Patient_id", "Patient_name", "Phone_no","organ_req","Location","Org_id"])
    ...
    if selected_result:
        ...
        if st.button("Update Patient"):
            edit_patient_data(new_patient_id,new_patient_name,new_phone_no,new_organ_req,new_location,new_org_id,Patient_id,Patient_name,Phone_no,organ_req,Location,Org_id)
            st.success("Successfully updated")

    result2 = view_all_data()
    df2 = pd.DataFrame(result2, columns=["Patient_id", "Patient_name", "Phone_no","Organ_req","Location","Org_id"])
